...
public class StreamingApp {
     ...
     * // NOTE: Checkpointing is automatically handled by AWS Managed Flink every
     * 60s.
     * 
     * String region = null;
     ...
     * appProps.get(StreamingApp.S3_SOURCE).getProperty(INPUT_BUCKET);
     * String table = appProps.get(S3_SOURCE).getProperty(TABLE,
     * DEFAULT_INPUT_TABLE);
     ...
     * String table = appProps.get(S3_SINK).getProperty(TABLE,
     ...
     * } catch (IOException e) {
     * LOG.
     * warn("Could not load application properties from KinesisAnalyticsRuntime: {}"
     * , e.getMessage());
     * }
     * 
     * if (StringUtils.isBlank(region) || StringUtils.isBlank(s3InputPath) ||
     * StringUtils.isBlank(s3OutputPath)) {
     * throw new
     * RuntimeException("One or more application properties or environment variable were not set and hence the application cannot run."
     * );
     * }
     * 
     ...
     * 
     * 
     * FileSource<String> fileSource = forRecordStreamFormat(new
     * TextLineInputFormat(), new Path(s3InputPath)).build();
     * DataStream<String> sourceRecords = env.fromSource(fileSource,
     * WatermarkStrategy.noWatermarks(), "S3-Data-Source");
     * 
     * // Parse the data to obtain a Tuple of (name,1) for each record
     * DataStream<Tuple2<String, Integer>> parsedData = sourceRecords.flatMap(new
     ...
     * // Visits per person by aggregating the Tuple2 of (name.1)
     * DataStream<Tuple2<String, Integer>> visitsPerPerson = parsedData.keyBy(value
     * -> value.f0).sum(1);
     * LOG.info("Visits per person: {}", visitsPerPerson);
     * 
     * // Tramsform to the Output Map
     * DataStream<String> results = visitsPerPerson.map(new
     ...
     * });
     * 
     * // Configure S3 sink with rolling policy. The Sink Rolling Policy specific
     * rules for when to stop writing to one file and start a new one.
     * // In this case, we are using a simple policy that starts writing to a new
     * file every 5s or if there is no activity for 3 seconds.
     * // If the sink doesn't receive any new data for 3 seconds, it will close the
     * current file. This is useful to prevent files from staying open for
     * // a long time if your data stream has quiet periods.
     * FileSink<String> s3Sink = FileSink.forRowFormat(new Path(s3OutputPath), new
     * SimpleStringEncoder<String>("UTF-8"))
     * .withRollingPolicy(DefaultRollingPolicy.builder().withRolloverInterval(
     * Duration.ofSeconds(5)).withInactivityInterval(Duration.ofSeconds(3)).build())
     * .build();
     * 
